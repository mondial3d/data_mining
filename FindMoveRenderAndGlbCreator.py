...
def fit_camera_to_objects(cam_obj, objects):
    ...
    # Frame all objects in the camera's view
...
def import_and_process(file_path, output_folder):
    try:
        ...
        # Adjust render settings for shadow quality
        bpy.context.scene.render.engine = 'BLENDER_EEVEE'  # or 'CYCLES'
        bpy.context.scene.eevee.shadow_cube_size = '1024'  # Increase for better shadow quality
        bpy.context.scene.eevee.shadow_cascade_size = '1024'
        bpy.context.scene.eevee.use_soft_shadows = True  # Enable soft shadows for realism
        bpy.context.scene.eevee.light_threshold = 0.01  # Lower threshold for more detailed lighting

        bpy.context.scene.render.resolution_x = 1920
        bpy.context.scene.render.resolution_y = 1080
        bpy.context.scene.render.image_settings.file_format = 'PNG'

         # Generate a large random string (e.g., 20 characters long)
        random_str = get_random_string(20)  # Adjust the length as needed

        # Create the base filename without an extension
        base_name = os.path.splitext(os.path.basename(file_path))[0]

        # Append the random string to the filename
        base_name_with_random = base_name + random_str

        # Render and save PNG
        render_output = os.path.join(output_folder, base_name_with_random + '.png')
        bpy.context.scene.render.filepath = render_output
        bpy.ops.render.render(write_still=True)

        # Export to GLB
        glb_output = os.path.join(output_folder, base_name_with_random + '.glb')
        bpy.ops.export_scene.gltf(filepath=glb_output)
...
output_folder = 'F:\\x1x1\\output'  # Update to your desired output folder
...
#blender --background --python c:\FindMoveRenderAndGlbCreator.py 